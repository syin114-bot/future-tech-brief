...
async function loadTranslations() {
  try {
    // 加时间戳防止浏览器缓存旧文件
    const response = await fetch('/i18n/translations.language-fix-test.json?v=' + Date.now());
...
function setLanguage(lang) {
  ...
  // 更新导航栏语言按钮
  const langBtn = document.getElementById('language-dropdown-btn') || document.getElementById('lang-btn');
  if (langBtn) {
    langBtn.innerHTML = `${LANGUAGE_FLAGS[lang]} ${SUPPORTED_LANGUAGES[lang]}`;
  }
...
function createLanguageButton() {
  ...
  button.innerHTML = `${LANGUAGE_FLAGS[currentLanguage]} ${SUPPORTED_LANGUAGES[currentLanguage]}`;
